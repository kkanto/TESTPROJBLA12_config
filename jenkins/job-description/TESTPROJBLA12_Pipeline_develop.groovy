...
def job_config = [
        job: [
                name: "TESTPROJBLA12_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "TESTPROJBLA12", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("TESTPROJBLA12_Pipeline_develop");
