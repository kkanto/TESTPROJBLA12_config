...
def job_config = [
    job: [
        name: "TESTPROJBLA12_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
def customLib = library identifier: 'TESTPROJBLA12_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/TESTPROJBLA12_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.TESTPROJBLA12.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "kkanto", "TESTPROJBLA12", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("TESTPROJBLA12_Feature_Build");
...
job.with {
    ...
    authenticationToken('so1aObJIE4x7jDdNLBNNfdTODJog6FVd')
